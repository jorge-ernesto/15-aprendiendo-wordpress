(function ($, wp_customize) {
	...
	var CustomizeBuilder = function (options, id) {
		var Builder = {
			...
			ready: false,
			devices: {
				desktop: CosmosWP_Customizer_Builder.desktop_label,
				mobile: CosmosWP_Customizer_Builder.mobile_tablet_label,
			},
			activePanel: 'desktop',
			panels: {},
			activeRow: 'main',
				...
				$('.cosmoswp-device-panel', that.container).each(function () {
					...
					$('.cosmoswp-available-items .grid-stack-item', panel).draggable({
						...
						start: function (event, ui) {
							...
							ui.helper
								.closest('.cosmoswp-available-items')
								.addClass('cwp-available-item-moving');
						},
						stop: function (event, ui) {
							$('body').removeClass('builder-item-moving');
							$('.cosmoswp-cb-items', panel).css('z-index', '');
							$('.cosmoswp-available-items', panel).removeClass(
								'cwp-available-item-moving'
							);
			...
			gridster: function ($wrapper, ui, event) {
				...
				if (!is_rtl) {
					...
					xi = Math.round(
						(wOffset.left + width - (iOffset.left + itemWidth + 10)) / colWidth
					);
					if (xi < 0) {
						xi = 0;
					}
			...
			addNewWidget: function ($item, row) {
				...
				var panel = that.container.find(
					'.cosmoswp-device-panel.cosmoswp-panel-' + that.activePanel
				);
				var el = row;
				if (!_.isObject(el)) {
			...
			addPanel: function (device) {
				...
				return (
					'<div class="cosmoswp-device-panel cosmoswp-vertical-panel cosmoswp-panel-' +
					device +
					'" data-device="' +
					device +
					'">' +
			...
			addDevicePanels: function () {
				...
				_.each(that.devices, function (device_name, device) {
					...
					$('.cosmoswp-cb-devices-switcher', that.container).append(
			...
			addAvailableItems: function () {
				...
				_.each(that.devices, function (device_name, device) {
					var $itemWrapper = $(
						'<div class="cosmoswp-available-items" data-device="' +
							device +
							'"></div>'
					);
					$('.cosmoswp-panel-' + device, that.container).append($itemWrapper);
					$itemWrapper.append(
						'<div class="cwp-close-item-wrap"><span class="cwp-close-item-panel dashicons dashicons-no"></span></div>'
					);
			...
			switchToDevice: function (device, toggle_button) {
				...
				if (numberDevices > 1) {
					$('.cosmoswp-cb-devices-switcher a', that.container).removeClass(
						'cosmoswp-tab-active'
					);
					$(
						'.cosmoswp-cb-devices-switcher .switch-to-' + device,
						that.container
					).addClass('cosmoswp-tab-active');
					$('.cosmoswp-device-panel', that.container).addClass(
						'cosmoswp-panel-hide'
					);
					$(
						'.cosmoswp-device-panel.cosmoswp-panel-' + device,
						that.container
					).removeClass('cosmoswp-panel-hide');
					that.activePanel = device;
				} else {
					$('.cosmoswp-cb-devices-switcher a', that.container).addClass(
						'cosmoswp-tab-active'
					);
				}
					...
					_.each(device_data, function (items, row_id) {
						if (!_.isUndefined(items)) {
							_.each(items, function (node, index) {
								var item = $(
									'.cosmoswp-available-items[data-device="' +
										device +
			...
			focus: function () {
				this.container.on(
					'click',
					'.cosmoswp-cb-item-setting, .cosmoswp-cb-item-name, .item-tooltip',
					function (e) {
						e.preventDefault();
			...
			remove: function () {
				...
				$document.on(
					'click',
					'.cosmoswp-device-panel .cosmoswp-cb-item-remove',
					function (e) {
						e.preventDefault();
			...
			showPanel: function () {
				var that = this;
				this.container
					.removeClass('cosmoswp-builder--hide')
					.addClass('cosmoswp-builder-show');
				setTimeout(function () {
			...
			init: function (controlId, items, devices) {
				...
				if (options.section) {
					wp_customize
						...
						.container.addClass('cosmoswp-hide');
				}
				...
				that.container.on(
					'click',
					'.cosmoswp-cb-devices-switcher a.switch-to',
					function (e) {
						e.preventDefault();
						var device = $(this).data('device');
						that.switchToDevice(device);
						$('.cosmoswp-cb-body')
							.find('.cosmoswp-available-items')
							.each(function () {
	...
	wp_customize.bind('ready', function (e, b) {
				...
				if (
					...
					$(
						...
						'<p class="footer_moved_widgets_text">' +
							CosmosWP_Customizer_Builder.footer_moved_widgets_text +
							'</p>'
					);
				}
			}, 1000);
		...
		wp_customize.state('expandedSection').bind(function (section) {
			...
			if (section) {
				$('.cosmoswp-cb-row[data-id="' + section.id + '"]').addClass(
					'row-active'
				);
				$(
					'.cosmoswp-device-panel .grid-stack-item.for-s-' + section.id
	...
	$document.on('mouseover', '.cosmoswp-cb-row .grid-stack-item', function (e) {
		var item = $(this);
		var nameW =
			$('.cosmoswp-cb-item-remove', item).outerWidth() +
			$('.cosmoswp-cb-item-setting', item).outerWidth();
	...
	$document.on('click', '.cwp-add-new-item', function (e) {
		...
		if (!this_item_wrap.length) {
			this_item_wrap = this_add_new
				.closest('.cosmoswp-cp-sidebar')
				.next('.cosmoswp-available-items');
		}
		if (this_item_wrap.length) {
	...
	function cwp_hide_item_panel(this_item_wrap) {
		...
		if (!this_add_new.length) {
			this_add_new = this_add_new
				.prev('.cosmoswp-cp-sidebar')
				.find('.cwp-add-new-item');
		}
		if (this_item_wrap.length) {
	...
	$document.on('click', '.cwp-close-item-panel', function (e) {
		var this_close_item_panel = $(this),
			this_item_wrap = this_close_item_panel.closest(
