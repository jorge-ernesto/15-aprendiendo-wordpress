...
wp.customize.controlConstructor['cosmoswp-responsive-buttonset'] = wp.customize.Control.extend(
	{
		...
		updateValue: function () {
			...
			var valueToPush = {};
			control.container.find( '.cosmoswp-responsive-buttonset-device-wrap' ).each(
				function(){
					var $         = jQuery;
...
function cwp_alpha_color_control( wrap, $ ){
	...
	wrap.find( '.cwp-alpha-color-control' ).each(
		function() {
...
wp.customize.controlConstructor[ 'responsive-range' ] = wp.customize.Control.extend(
	{
		...
		resetValues: function () {
			'use strict';
			this.container.find( '.responsive-range--input' ).each(
				function () {
					jQuery( this ).val( jQuery( this ).data( 'default' ) );
				}
			);

			this.container.find( '.responsive-range--select' ).each(
				function () {
...
wp.customize.controlConstructor['cosmoswp-repeater'] = wp.customize.Control.extend(
	{
		ready: function () {
				...
				function(){
					...
					if (typeof $this !== 'undefined') {
						...
						if (typeof field !== 'undefined') {
							field.find( "input[type='text'][data-single-name]" ).each(
								function(){
									var defaultValue = $( this ).attr( 'data-default' );
									$( this ).val( defaultValue );
								}
							);
							field.find( "textarea[data-single-name]" ).each(
								function(){
									var defaultValue = $( this ).attr( 'data-default' );
									$( this ).val( defaultValue );
								}
							);
							field.find( "select[data-single-name]" ).each(
								function(){
									var defaultValue = $( this ).attr( 'data-default' );
									$( this ).val( defaultValue );
								}
			...
			this.container.on(
				...
				function(){
					...
					if ( typeof	$( this ).parent() !== 'undefined') {
						$( this ).closest( '.repeater-field-control' ).slideUp(
							'normal',
							function(){
								$( this ).remove();
								control.updateValue();
			...
			this.container.find(".cosmoswp-repeater-field-control-wrap .contact-link-type select[data-single-name]").change(function () {

				var $     = jQuery;
				if ($( this ).val() === 'disable') {
					console.log("im inside")
					$( this ).parent().siblings('.contact-url').addClass("cosmoswp-hide-element");
					$( this ).parent().siblings('.contact-checkbox').addClass("cosmoswp-hide-element");
				} else {
					$( this ).parent().siblings('.contact-url').removeClass("cosmoswp-hide-element");
					$( this ).parent().siblings('.contact-checkbox').removeClass("cosmoswp-hide-element");
				}
			}).trigger( "change" );
		},
		...
		updateValue: function () {
			...
			var control = this,
			values      = [];
			control.container.find( ".repeater-field-control" ).each(
				function(){
					var $           = jQuery;
					var valueToPush = {};
					$( this ).find( '[data-single-name]' ).each(
						function(){
							if ( $( this ).attr( 'type' ) === 'checkbox') {
								if ($( this ).is( ':checked' )) {
									var dataValue = 1;
...
wp.customize.controlConstructor['cosmoswp-slider'] = wp.customize.Control.extend(
	{
		...
		ready: function() {
			...
			function set_value(){
				var $           = jQuery;
				var valueToPush = {};
				control_container.find( '.slider-input' ).each(
					function () {
						var device = $( this ).attr( 'data-device' );
						if (typeof valueToPush[device] === 'undefined') {
							valueToPush[device] = {};
	...
	$( document ).ready(
		function () {
			...
			function refresh_tabs_cssbox_values( control_cssbox_wrap ){
				var valueToPush = {};
				control_cssbox_wrap.find( '.tabscssbox-field' ).each(
					function(){
			...
			function select_parent_type_result(mode,parent,selected_val){
				if (selected_val === 'none') {
					parent.siblings().find( '.customize-control-cosmoswp-cssbox' ).each(
						function () {
					...
					parent.siblings().find( '.customize-control-alpha-color' ).each(
						function () {
					...
					if ( tabs_field.hasClass( 'tabscssbox-field' ) ) {
						if ( ! tabs_field.hasClass( 'tabscssbox_link' ) ) {
							...
							if ( device_wrap.find( '.tabscssbox_link' ).is( ':checked' ) ) {
								device_wrap.find( '.tabscssbox-field' ).each(
									function(){
										$( this ).val( dataValue );
									}
								);
			...
			customize_theme_controls.find( '.tabs-field-control-wrap select[data-single-name]' ).each(
				function(){
					cosmoswp_tab_field_border_style_control( $( this ) )
				}
			);

			$( '.tabs-field-control' ).on(
				'click',
				'.tab-trigger',
				function () {
	...
	function($) {
		...
		customize_theme_controls.on(
			...
			function() {
				...
				icons_list_wrapper.find( 'i' ).each(
					function () {
			...
			function(){
				...
				if ((typeof(image_src) !== 'undefined' && image_src != null ) ) {
					...
					if (overlay_checkbox_attr === 'enable-overlay') {
						...
						if (selected_val) {
							image_parent.siblings().find( '.customize-control-alpha-color' ).each(
								function () {
									var color = $( this ).data( 'color-single-name' );
									if (color === 'background-overlay-color') {
										$( this ).parent().show();
				...
				$( ".customize-control-group .cosmoswp-image-remove" ).click(
					function(){
						...
						this_remove.parent().siblings().find( '.customize-control-alpha-color' ).each(
							function () {
								var color = $( this ).data( 'color-single-name' );
								if (color === 'background-overlay-color') {
									  $( this ).parent().hide();
		...
		function load_google_font_weight( parent_group_fields, selected_google_font ){
			...
			if ( google_webfonts_json_data_items.length ) {
				$.map(
					google_webfonts_json_data_items,
					function( item, i ) {
						selected    = '';
		...
		$( "select[data-single-name='font-type']" ).on(
			"change rightnow",
			function() {
				var this_font       = $( this ),
